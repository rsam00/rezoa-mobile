...
async function importUsTop100() {
  console.log('🚀 Starting US Top 100 Import...');
  ...
  try {
    ...
    // 3. Fetch Top Clicks and Top Votes specifically for the US
    console.log(`📥 Fetching Top 100 US Stations by Clicks...`);
    const usClicksUrl = `https://${serverHostname}/json/stations/search?countrycode=US&limit=100&order=clickcount&reverse=true&hidebroken=true`;
    const topClicks = await fetchJson(usClicksUrl);
    
    console.log(`📥 Fetching Top 100 US Stations by Votes...`);
    const usVotesUrl = `https://${serverHostname}/json/stations/search?countrycode=US&limit=100&order=votes&reverse=true&hidebroken=true`;
    const topVotes = await fetchJson(usVotesUrl);
    ...
    console.log(`   - Total unique US top stations fetched: ${fetchedCount}`);
    ...
    if (formattedStations.length === 0) {
      console.log('\n🛑 All fetched US top stations already exist in the database! Nothing to do.');
      return;
    }

    console.log(`\n💾 Upserting ${formattedStations.length} new US stations into Supabase...`);
    const { error } = await supabase.from('stations').upsert(formattedStations, { onConflict: 'id' });
    
    if (error) throw error;
    
    console.log(`🎉 Successfully imported ${formattedStations.length} US Top stations! They are now live in the app.`);
